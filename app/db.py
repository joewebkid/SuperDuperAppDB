...
# Defaults preserve the existing moderators and include the repository owner.
# Production can replace the entire set through APPDB_ADMIN_GITHUB_LOGINS.
DEFAULT_ADMIN_GITHUB_LOGINS: frozenset[str] = frozenset(
    {"joewebkid", "timofeylednev", "j92580498-max", "makaherpasswordlostmedia"}
)


def admin_github_logins() -> frozenset[str]:
    configured = os.environ.get("APPDB_ADMIN_GITHUB_LOGINS")
    if configured is None:
        return DEFAULT_ADMIN_GITHUB_LOGINS
    return frozenset(
        login.strip().lower()
...
class User(Base):
    ...
    (display username; may change). ``is_admin`` is recomputed from the
    configured administrator login set on every sign-in.
    """
...
class Report(Base):
    ...
    # New deployments keep uploaded screenshots in the database. Free web
    # hosts usually provide an ephemeral filesystem, so a file saved beside
    # the application would disappear after a restart or redeploy. The legacy
    # filename column remains readable for existing self-hosted installations.
    screenshot_blob: Mapped[bytes | None] = mapped_column(
...
class PatchSubmission(Base):
    """Anonymous, deduplicated patch handoff to the GitHub review queue."""
    ...
    patch_id: Mapped[str] = mapped_column(String(96), nullable=False, index=True)
    bundle_identifier: Mapped[str] = mapped_column(String(200), nullable=False, index=True)
...
def init_db() -> None:
    ...
    with engine.begin() as conn:
        ...
        if "screenshot_blob" not in report_cols:
            conn.exec_driver_sql(
                "ALTER TABLE reports ADD COLUMN screenshot_blob BLOB"
            )
        if "screenshot_content_type" not in report_cols:
            conn.exec_driver_sql(
                "ALTER TABLE reports ADD COLUMN screenshot_content_type VARCHAR(50)"
            )
