...
from .patch_submissions import PatchSubmissionRequest, submit_patch
...
FORM_FIELD_LIMITS = {
    "new_app_name": 200,
    "new_app_year": 4,
    "new_app_publisher": 200,
    "version_number": 40,
    "display_name": 120,
    "bundle_identifier": 200,
    "minimum_ios_version": 20,
    "touchhle_version": 80,
...
# only from explicitly configured proxies. Trusting every source lets a direct
# client forge the callback scheme and host.
_trusted_proxy_hosts = [
    host.strip()
    for host in os.environ.get("TRUSTED_PROXY_HOSTS", "127.0.0.1,::1").split(",")
    if host.strip()
]
...
_configured_session_secret = os.environ.get("SESSION_SECRET")
if _is_production and (
    not _configured_session_secret or len(_configured_session_secret) < 32
):
    raise RuntimeError(
        "SESSION_SECRET must contain at least 32 characters when APP_ENV=production"
    )
_session_secret = _configured_session_secret or py_secrets.token_urlsafe(48)
app.add_middleware(
    SessionMiddleware,
    secret_key=_session_secret,
    session_cookie="hyperhle_session",
    https_only=_env_bool("SESSION_HTTPS_ONLY", _is_production),
    same_site="lax",
)
...
def _startup() -> None:
    init_db()
    if _env_bool("SEED_DEMO_DATA", not _is_production):
        seed()
...
@app.post("/api/patch-submissions", status_code=202)
async def api_patch_submission(
    request: Request,
    payload: Annotated[PatchSubmissionRequest, Body()],
...
async def github_callback(
    ...
    if error:
        handle_authorization_error(request, state)
...
def logout_post(request: Request, csrf_token: Annotated[str, Form()]):
...
def _render_login_required(
    ...
    *,
    ...
    return templates.TemplateResponse(
        request,
        "login_required.html",
        {**ctx, "next_path": safe_next_path(next_path) or "/submit"},
        status_code=status_code,
    )
...
def submit_form(
    ...
    if user is None:
        return _render_login_required(request, db, next_path=request_target)
    form = _empty_form_data()
    if app_id is not None:
        form["app_id"] = str(app_id)
    for key, limit in FORM_FIELD_LIMITS.items():
        value = request.query_params.get(key)
        if value is not None:
            form[key] = value[:limit]
    selected_app_id = app_id
    bundle_identifier = form.get("bundle_identifier")
    if selected_app_id is None and bundle_identifier:
        matching_report = (
            db.query(Report)
            .filter(Report.bundle_identifier == bundle_identifier)
            .order_by(Report.reported_at.desc())
            .first()
        )
        if matching_report is not None:
            selected_app_id = matching_report.app_id
            form["app_id"] = str(selected_app_id)
    if selected_app_id is None and form.get("new_app_name"):
        matching_app = db.query(App).filter(App.name == form["new_app_name"]).first()
        if matching_app is not None:
            selected_app_id = matching_app.id
            form["app_id"] = str(selected_app_id)
        else:
            form["app_id"] = "__new__"
    return _render_submit_form(
        request, db, selected_app_id=selected_app_id, form_data=form,
    )
...
async def _read_uploaded_screenshot(
    upload: UploadFile | None,
) -> tuple[bytes | None, str | None, str | None]:
    """Validate an uploaded screenshot and return ``(bytes, MIME, error)``."""
    ...
    if len(blob) > MAX_SCREENSHOT_BYTES:
        return None, None, (
            f"Screenshot too large (max {MAX_SCREENSHOT_BYTES // (1024 * 1024)} MB)."
        )
    signatures = {
        ...
        ".webp": len(blob) >= 12 and blob.startswith(b"RIFF") and blob[8:12] == b"WEBP",
    }
    if not signatures.get(ext, False):
        return None, None, "Screenshot contents do not match its image extension."
    return blob, _IMAGE_CONTENT_TYPES[ext], None


async def _save_uploaded_screenshot(upload: UploadFile | None) -> tuple[str | None, str | None]:
    """Save a legacy filesystem screenshot to the uploads directory.

    Existing self-hosted installations may still use this helper. New report
    submissions store the validated bytes in the database instead.
    """
    blob, _content_type, error = await _read_uploaded_screenshot(upload)
...
@app.get("/screenshots/{report_id}")
def report_screenshot(
    report_id: int,
    request: Request,
    db: Annotated[Session, Depends(get_db)] = None,
):
    """Serve a database-backed screenshot with report visibility rules."""
    user = auth_module.get_current_user(request, db)
    report = db.query(Report).filter(Report.id == report_id).first()
    if report is None or report.screenshot_blob is None:
        raise HTTPException(404, "Screenshot not found")
    can_view = report.status == STATUS_APPROVED or (
        user is not None
        and (user.is_admin or report.reporter_user_id == user.id)
    )
    if not can_view:
        raise HTTPException(404, "Screenshot not found")
    cache_control = "public, max-age=86400" if report.status == STATUS_APPROVED else "no-store"
    return Response(
        content=report.screenshot_blob,
...
async def submit_post(
    ...
    for field, limit in FORM_FIELD_LIMITS.items():
        if len(form_data.get(field, "")) > limit:
            label = field.replace("_", " ").title()
            return _err(f"{label} is too long (max {limit}).")

    clean_screenshot_url = _clean(screenshot_url)
    if clean_screenshot_url:
        parsed_screenshot_url = urlsplit(clean_screenshot_url)
        if (
            parsed_screenshot_url.scheme != "https"
            or not parsed_screenshot_url.hostname
            or parsed_screenshot_url.username
            or parsed_screenshot_url.password
        ):
            return _err("Screenshot URL must be a public HTTPS URL without credentials.")

    if not user.is_admin:
        report_limit = _bounded_int_env("REPORTS_PER_USER_PER_HOUR", 10, 1, 100)
        recent_reports = (
            db.query(Report)
            .filter(Report.reporter_user_id == user.id)
            .filter(Report.reported_at >= utcnow() - timedelta(hours=1))
            .count()
        )
        if recent_reports >= report_limit:
            return _render_submit_form(
                request,
                db,
                form_data=form_data,
    ...
    screenshot_blob, screenshot_content_type, screenshot_err = (
        await _read_uploaded_screenshot(screenshot_file)
    )
    if screenshot_err:
...
def _serialise_report(r: Report, *, full: bool) -> dict:
    ...
    if full:
        base.update({
            ...
            "screenshot_path": (
                f"/screenshots/{r.id}" if r.screenshot_content_type else None
            ),
...
def _is_claim_stale(claimed_at: datetime | None) -> bool:
    if claimed_at is None:
        return True
    age = (utcnow() - claimed_at).total_seconds()
